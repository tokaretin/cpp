...
int main()
{
    ...
    std::cout << "arr[0] = " << arr << std::endl; // Выводим адрес начала массива arr. Он будет выведен в шестнадцатеричной форме.
    std::cout << "arr[1]= " << arr + 1 << std::endl;
    std::cout << "1-й элемент массива: " << *arr << std::endl;            // Выводим значение первого элемента массива arr. *arr разыменовывает указатель на начало массива.
    std::cout << "2-й элемент массива: " << *arr + 1 << std::endl;        // Вычисляем значение выражения *arr + 1, которое равно первому элементу массива плюс 1.

    int *parr = arr + 2;                                                // Создаем указатель parr и инициализируем его адресом третьего элемента массива arr (с индексом 2).
    std::cout << "int* parr = arr + 2 получаем " << *parr << std::endl; // Выводим значение элемента массива, на который указывает parr, то есть третий элемент arr.
    ...
    std::cout << "arr[2] = " << arr + 2  << std::endl << "arr[3]= "  << arr + 4 << std::endl; // arr[2] = 0x7d05dffa28 arr[3]= 0x7d05dffa30
   ...
   // за пределами массива будет выходить мусор. Так что нужно пользоваться крайне остарожно и пользоваться только вделеной памятью
    double* moremax = ar + 10;
    std::cout << moremax << " = " << *moremax << std::endl;
